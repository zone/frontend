# @zonedigital/jarvis

[![Part of Zone Frontend][zone-fe-image]][zone-fe-url] [![npm][npm-image]][npm-url] [![downloads][downloads-image]][npm-url]

[zone-fe-image]: https://img.shields.io/badge/-frontend-lightgrey.svg?logo=data:image/svg+xml;base64,&longCache=true&style=flat-square&colorA=2C2B39&colorB=1010E5
[zone-fe-url]: https://github.com/zone/frontend
[npm-image]: https://img.shields.io/npm/v/@zonedigital/jarvis.svg?style=flat-square
[npm-url]: https://npmjs.org/package/@zonedigital/jarvis
[downloads-image]: https://img.shields.io/npm/dm/@zonedigital/jarvis.svg?style=flat-square

This package provides Zone's JavaScript style guide as an extensible shared config.

## Requirements

You'll need to have at least NPM verison 5.2.0 installed to use NPX.

## Usage

You can run Jarvis with NPX

`npx @zonedigital/jarvis`
